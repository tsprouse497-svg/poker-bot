...
Proposed as phase 10 in `docs/V2_ROADMAP.md`: run the solver, commit the export, and have a human look at the range grids and say whether the extraction is faithful.
It leads the sequence as the deliberate exception to format-before-data, because an export is written in the solver's own vocabulary and so does not wait on the format work.
