...
The corpus comparison this phase used to close with is **phase 17**, split off by Taylor on 2026-08-31:
committing an artifact and rendering a verdict on it are different jobs, and one contract could not hold
both inside its cap. Three rulings shape what gets committed here. The criteria below enforce them and
the decision record argues them; neither the argument nor its measurements are repeated here.
**Decision 14** re-solves with `add_allin: false` at `0.00016`, giving **33,969 action nodes, 72,798**
...
strategy is backward-induced over every terminal below it, per
...
- **Hero's four-betting range at each committed three-bet spot is printed by class with arriving reach
  and read by a human before stage 6 freezes it**, the same way the defence level is. Those 15 spots
  exist to answer "do I four-bet", they weigh that branch entirely on the terminal the fit has no cell
  for, and on the only calibrated artifact measured the answer four-bets **87s at 0.892 and 76s at 0.534
  while JJ four-bets 0.000 and flats 0.999**, a range 23.3 percent suited connectors against 3.4 percent
  wheel aces. A shape like that is a halt and a decision, not a caveat.
- **The group gate is a regression detector and the contract says so, because it cannot tell a good
  chart from a flat one.** Measured chart-wide at decision 10's one-point tolerance the build this phase
  commits fails it - 4 pair inversions worst 7.40 points, 3 suited-row worst 23.19 - while the rejected
  `static` build passes at 0.06 and 0.00 and a chart edited to fold nothing scores zero and zero; a
  suited-offsuit transposition makes the row ladder cleaner. So the phase states what it does about
  failing its own gate rather than re-deriving a tolerance that admits the artifact it judges, and no
  reader may take a pass as evidence the ranges are sound.
...
- The report shows a non-coding reviewer, without reading code: the node census with reasons, one cell
  ...
  against the GTO Wizard expectations, and the withheld jams with AA's weight at each. At least one
  number is recomputable by hand from a committed file, and the audit packet says which and how.
- The report generator validates its own figures and exits non-zero when they do not hold: the node
  census, the artifact's spot count against the walk's, and the dominance relations. The rest is prose
  and the audit packet says so.
...
- The corpus-facing entries move to phase 17 with the measurement that settles them:
  `CORPUS-CALL-AGREEMENT-IS-THE-WEAK-SPOT`, `AGREEMENT-RATE-REWARDS-AN-UNCONVERGED-CHART`,
  `CHART-CANNOT-ANSWER-A-LIMPED-POT` and `CORPUS-INVENTORY-SHOULD-DRIVE-CHART-WORK`. This phase neither
  closes nor restates them.
...
- `REALIZATION-MODEL-UNDERPRICES-POSITION` closes on one of its three dispositions, and
  `CALIBRATED-REALIZATION-CARRIES-ITS-TRAINING-RAKE` is stated on the source card rather than closed.
  `PHASE-14-CONTRACT-DOES-NOT-FIT-ITS-OWN-CAP` closes on the split that created phase 17.
- `CHART-COVERAGE-EXPANSION` and `ARTIFACT-SIZE-LIMIT-VERSUS-SOLVE-COVERAGE` are restated rather than
  closed: the first is now mostly blocked on GTOpen, and the second must not keep a headroom argument
  nothing rests on.
...
- The four-bet range at each committed three-bet spot and the defence level per opener, as the two reads
  a human owes before the artifact is frozen, with what each showed.
