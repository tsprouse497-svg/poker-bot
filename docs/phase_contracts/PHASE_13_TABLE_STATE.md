...
## Scope
Five changes to what `StrategyQuery` carries about the table, and nothing to what a chart can
express. All five were `phase: "13"` entries in `backlog.yml` when this contract was written
against them; three closed there, and `ASYMMETRIC-EFFECTIVE-STACKS` and `BLIND-STRUCTURE-VARIANTS`
now read `phase: "14"`, each asking for a format change this phase is scoped out of.
...
- `BLIND-STRUCTURE-VARIANTS`: the query cannot see a straddle or an ante, so `PreflopChartStrategy`
  detects both by an arithmetic bound on the pot that a straddled pot with callers slips through.
...
This phase commits no artifact, no chart, and no new sample. It changes the runtime query and what
...
parser rejects anything else, the engine posts only two blinds, and the simulator gives every seat
one starting stack. The corpus therefore holds no asymmetric, straddled or anted table and every
table-shape count against it is zero before anything is measured, which is filed as
`CORPUS-CANNOT-EXPRESS-A-TABLE-SHAPE` and is not this phase's to fix. So constructed fixtures and
the strategy-report probes carry the discovery, the corpus carries a zero-delta regression proof,
and a criterion promising a corpus discovery here would promise a number that cannot exist.
...
- `pot` is validated against the contributions rather than trusted, and a query whose pot is not the
  sum of what the seats put in is rejected. Both live producers already build the pot as that sum,
  so the check is a tautology there, which the phase states rather than claiming an oracle it lacks;
  it bites at report producers that supply an independent pot, one of which attributes 100 chips to
  no seat, and at every producer written after this phase. A pot holding chips no seat contributed
  is rejected, not mis-derived, a hard stop for later ingestion of raked or dead-blind hands.
...
- `to_call` is capped at hero's remaining stack at every producer, which is Taylor's ruling of
  ...
  entry says three producers are uncapped, and stage 1 measured nine construction sites across six
  files of which two cap. Two the entry does not name are load-bearing, so they are named here. The
  preflop strategy report builds its straddle probe by overriding the bet level alone and its ante
  probe by overriding the pot alone, so under this phase's rules the first stops refusing and the
  second cannot be built at all; both are rebuilt to carry per-seat forced chips so that report
  keeps the two refusals it exists to show. The postflop fallback enumeration attributes 100 chips
  to no seat, so the phase decides where that dead money sits rather than relaxing the
...
- The two all-in ceilings become one. `PreflopChartStrategy` caps its raise at the street's bet
  ...
  stack, so the chart's is higher by what hero still owes to match the level - an uncapped
  quantity, which `to_call` stops being the moment hero is capped. Both are now expressed from
  hero's recorded contribution, and a test pins they agree on a hero who has already invested.
- Phase 06's short hero is restated. `0 < stack < to_call` is unsatisfiable once `to_call` is
  capped, surviving as `to_call == stack`. The enumeration still covers it and its contract says so.
...
- The flat-table test runs in both directions and over live seats only. Today only a seat deeper
  than hero refuses; a seat shallower than hero is invisible and the chart answers at hero's depth.
  Both now refuse, and a shallower seat gets a refusal code of its own, because "somebody is short"
  and "somebody is deep" are different tables and a single code would merge them in the inventory.
...
- On the committed corpus every count above is expected to be zero, and the phase reports those
  zeros as a checked regression proof: a corpus number that moves is a defect, not a discovery. The
  discovery evidence is constructed fixtures, and the report states for each what the chart derived
  before and now. One committed fixture moves, the phase's only live evidence: the three-handed
  side-pot hand at 50, 100 and 200 chips changes refusal code in the postflop fallback report.

### A straddle and an ante are detected exactly, not bounded
- Forced money is found by reconstructing what each seat should hold from the two declared blinds
  and its own recorded actions, and comparing that to what it holds. A seat holding more than the
  reconstruction predicts carries chips it never chose to put in. That is exact where the pot bound
  it replaces is generous, and it always catches an ante, including on a seat that has since folded.
...
- The case the bound admits is pinned by test as detected: 50/100 with a 200 straddle, a raise to
  600, the straddler and the big blind calling, the small blind folding. Its pot of 1,850 sits
  under the bound's 1,950 and slips through, its contributions match an unstraddled pot at the same
  price, and the minimum raise target is what gives it away. It is the case
  `PER-SEAT-CONTRIBUTIONS-IN-QUERY` names, and it is common in the home games this bot targets.
...
- The serialized decision-audit key changes with the field, so `DECISION_AUDIT_SCHEMA_VERSION`
  moves and the audit packet says why. Two payload shapes under one version number is
  `DECISION-AUDIT-VERSION-SPANS-TWO-STREET-BET-READINGS`, which this repo has already recorded once.
- No completed contract is left naming a field that no longer exists. Phase 03 is amended at stage
  1 in the two-line form `AGENTS.md` requires. Phase 11 names the field in several criteria and
  separately asserts that the preflop chart's raise cap is its own all-in target, which the ceiling
  criterion above makes false. It sat at exactly the 300-line cap and could take no amendment, and
  `AGENTS.md` rules that case a rewrite as its own `contract-update` task, so the task is
  `ENGINE-FIDELITY-CONTRACT-IS-AT-ITS-LINE-CAP` and this phase does not tag until it has run.
...
- No frozen assertion is weakened to fit. A test that asserted the old field name asserts the new
  one; one that asserted behaviour now wrong is rewritten to the claim it was making, and the audit
  packet says which and why. That includes the frozen test asserting `StrategyQuery`'s class
  docstring, rewritten to the fields the query now has rather than contorted to satisfy its test.
...
- A call entry records no all-in-for-less, so a short all-in call and a full call are one spot.
  Same treatment: measured, then closed here or restated as a key change with its cost.
...
- The judgment calls in `reports/phase_audits/decisions/PHASE_13_TABLE_STATE_DECISIONS.md` carry a
  reversibility class before implementation begins and the packet records each outcome.
  `frozen-into-data` reaches a fixture as well as an artifact, so a behaviour default this contract
  requires a frozen test to pin carries the class even though the phase commits no artifact.
