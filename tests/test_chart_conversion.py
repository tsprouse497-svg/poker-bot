...
on each, keyed by what hero faces and by hero's seat; the six-rung price ladder; the proof
...
HERO_PRICES = (2.5, 7.5, 13.5, 22.5, 40.5, 100.0)
"""**Six rungs after MAINT-34, not three, and the branch is by seat.** A re-raise is the
current bet times the raiser's own multiplier - the global 3.0, or 5.4 where
`raise_mults_by_seat` names a blind - so a three-bet is 7.5 in position and 13.5 from a blind
and the four-bet over it 22.5 or 40.5. A blind four-betting the other blind's 13.5 reaches
72.9, which `allin_threshold` snaps to the stack, so 100.0 is a committed price now. Derived
from `RULED_CONFIG` below rather than trusted here."""

PRICES_BY_RAISES_FACED = {0: {2.5}, 1: {7.5, 13.5}, 2: {22.5, 40.5, 100.0}}
"""A spot's price is a function of what hero faces **and of hero's own seat**, and of nothing
else - not of how many cold callers are in. The seat term is what MAINT-34 added; before it
...
def highest_raise_to(
    by_path: dict[tuple[int, ...], SolverNode], node: SolverNode
) -> float | None:
    """The largest amount anybody has raised to on the line into a node, or None first in.

    The multiplier applies to the current bet rather than to the opening size, so this is the
    other half of rebuilding a price from `RULED_CONFIG`. Read off the sequence rather than off
    the node, which carries only what hero may do next.
    """
    raised = [
        entry.size_bb
        for entry in shape.action_sequence_of(by_path, node)
        if entry.action == "raise" and entry.size_bb is not None
    ]
    return max(raised) if raised else None
...
def test_the_price_ladder_is_the_one_the_solved_config_produces(
    ...
    """Prices derived rather than listed, and keyed by what hero faces and by hero's own seat.

    A re-raise is the amount already raised to times the raiser's own multiplier: the global
    `raise_mults`, or the blind's own menu where `raise_mults_by_seat` names one. A raise that
    reaches `allin_threshold * stack` snaps to the stack. Every committed price is rebuilt from
    those four config fields here, so a re-solve at the ruled config cannot move one without
    moving the config, and a test that spelled the six numbers would be pinning a coincidence.

    **One price per spot is still asserted, not assumed.** It is what makes the price a function
    of the spot key; a spot offering two would be the case decision 6's schema exists for, and
    ...
    menus = RULED_CONFIG["raise_mults_by_seat"]
    clamp = float(RULED_CONFIG["allin_threshold"]) * float(RULED_CONFIG["stack"])

    def multiplier_for(seat: str) -> float:
        own = menus[seats.index(seat)]
        return float(own[0]) if own else float(RULED_CONFIG["raise_mults"][0])
    ...
    for node in keyed.values():
        ...
        raised_to = highest_raise_to(by_path, node)
        if raised_to is None:
            expected = open_to
        else:
            expected = raised_to * multiplier_for(node.actor_pos)
            if expected >= clamp:
                expected = STACK_BB

        assert len(prices) == 1, (faced, prices)
        assert prices[0] == pytest.approx(expected), (faced, node.actor_pos, raised_to)
        by_faced.setdefault(faced, set()).add(prices[0])

    quoted = {price for prices in by_faced.values() for price in prices}

    assert by_faced == PRICES_BY_RAISES_FACED
...
def test_heros_own_jam_lives_only_at_the_four_bet_facing_spots_the_phase_excludes(
    ...
    """The containment half of the canary, which MAINT-34 turned from a prohibition into a count.

    The canary that rejected the first cutover caught a chart telling hero to put a hundred
    blinds in where the solve raises to seven and a half. That is still the failure being
    ...
    asked to *answer* a jam - no `@100` key reaches the chart or its table.
    """
    jam_nodes = [n for n in export.nodes if any(a.kind == "jam" for a in n.actions)]
    keyed = shape.committed_nodes(export, by_path)
    committed_jams = [n for n in jam_nodes if n.path in {c.path for c in keyed.values()}]

    assert len(jam_nodes) == JAM_NODES
    assert dict(Counter(shape.raises_faced_of(by_path, n) for n in jam_nodes)) == JAM_RAISES_FACED
    assert len(committed_jams) == COMMITTED_JAM_NODES
    for node in committed_jams:
        assert shape.raises_faced_of(by_path, node) == 2, node.path
        assert node.actor_pos in ("SB", "BB"), node.path
        assert highest_raise_to(by_path, node) == pytest.approx(13.5), node.path
...
def test_no_committed_node_offers_a_jam_beside_a_named_raise(export, by_path) -> None:
    ...
    not sum to one. No committed node offers both, so the collapse never fires and the
    ...
    *replaces* the named four-bet rather than adding a jam beside it. Every committed jamming
    node offers exactly fold, call, jam, which is asserted rather than assumed: a build that
    kept both would put two aggressive prices in one cell, and that is the defect.
    """
    keyed = shape.committed_nodes(export, by_path)
    ...
    assert len(keyed) == SPOTS_OFFERING_A_PRICE
    assert len(jamming) == COMMITTED_JAM_NODES
    assert both == set()
    for key in jamming:
        assert tuple(a.kind for a in keyed[key].actions) == ("fold", "call", "jam"), key
    shape.vacuous("no committed node offers a jam beside a raise, so the collapse never fires")
