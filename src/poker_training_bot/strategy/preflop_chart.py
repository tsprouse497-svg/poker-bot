...
REFUSE_TIE = f"{CODE_PREFIX}:weight-tie"
...
class PreflopChartStrategy:
    ...
    def collapse(self, weights: tuple[tuple[str, float], ...]) -> str | None:
        """The single action a mixed strategy resolves to, or None on a tie.

        Highest weight wins. A tie returns None so the caller refuses, because
        breaking it would mean inventing a preference the chart does not express,
        and doing so silently is how an unbacked decision looks exactly like a
        backed one.
        """
        positive = [(action, weight) for action, weight in weights if weight > 0.0]
        if not positive:
            return None
        best = max(weight for _, weight in positive)
        leaders = [action for action, weight in positive if weight == best]
        return leaders[0] if len(leaders) == 1 else None

    def _rationale(self, action: str, hit: ChartHit) -> str:
        vector = ",".join(f"{name}={weight:g}" for name, weight in hit.action_weights)
        return f"{CODE_PREFIX}:highest-weight:{action}[{vector}]"
    ...
    def decide(self, query: StrategyQuery) -> StrategyDecision | StrategyRefusal:
        ...
        action = self.collapse(found.action_weights)
        if action is None:
            return StrategyRefusal(REFUSE_TIE)
        if action not in query.legal_actions:
            return StrategyRefusal(REFUSE_ILLEGAL)
        if action != "raise":
            return StrategyDecision(action, None, self._rationale(action, found))

        amount, refusal_code = self._raise_amount(query, found.spot_key)
        if amount is None:
            return StrategyRefusal(refusal_code or REFUSE_NO_SIZING)
        return StrategyDecision("raise", amount, self._rationale("raise", found))
    ...
    def decide_spot(
        ...
        action = self.collapse(weights)
        if action is None:
            return StrategyRefusal(REFUSE_TIE)
        vector = ",".join(f"{name}={weight:g}" for name, weight in weights)
        code = f"{CODE_PREFIX}:highest-weight:{action}[{vector}]"
        if action != "raise":
