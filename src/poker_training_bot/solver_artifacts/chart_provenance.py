...
from poker_training_bot.solver_artifacts.chart_provenance_facts import (
    _aggressive_prices,
    _closes_into_a_multiway_flop,
    _combo_weighted_fold_pct,
    _committed,
    _decision_shares,
    _derivation,
    _faced_price,
...
from poker_training_bot.solver_artifacts.gtopen_export import gtopen_class_index
...
def _exclusion_clauses(by_path: _ByPath, census: NodeCensus) -> str:
    ...
    parts: list[str] = []
    # Every code in the vocabulary needs a branch here or the note under-reports in silence,
    # which is what happened when MAINT-34's clause five arrived: the paragraph said "5 clauses
    # can refuse a node and 4 of them do here" while 128 nodes sat under the fifth.
    assert len(DERIVATION_EXCLUSION_CODES) == 5, (
        "an exclusion code was added without a sentence here; the note would count it in the"
        " clause total and then not describe it"
    )
    ...
    unclosed = counted.get(DERIVATION_TERMINAL_SPLIT_DOES_NOT_CLOSE, 0)
    if unclosed:
        blind = sum(
            1
            for node in by_path.values()
            if exclusion_code(by_path, node) == DERIVATION_TERMINAL_SPLIT_DOES_NOT_CLOSE
            and multiway_exposure_pct(by_path, node) == 0.0
        )
        parts.append(
            f"{unclosed:,} nodes clear every other clause and their exposure could not be"
            " measured: mass flowing into a node no hand class arrives at leaves the walk"
            f" without being redistributed, so the three shares do not close on 100. At {blind:,}"
            " of them every branch evaporated and the measurement read exactly zero, which the"
            " clause would have admitted as heads-up. A guard that cannot see its own input"
            " fails closed."
        )
