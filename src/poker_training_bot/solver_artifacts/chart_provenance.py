...
ARTIFACT_NOTES = (
    ...
    " open somebody has already called. Those 10 are refused by a rule naming the seat"
    " rather than by the exposure measurement, which cannot reach them: the big blind"
    " folds better than nine times in ten there, so its call branch carries too little"
    " mass to trip the threshold even though what that call buys is a three-way pot every"
    " time. They are the only shape a seat rule was needed for. Elsewhere in the chart 58"
    " committed spots do publish a call that closes into a guaranteed three-or-more-way"
    " flop, every one of them facing a three-bet, and those are kept on purpose - the"
    " paragraph below says why a call to a three-bet is not a cold call."
    "\n\n"
    ...
    "\n\n"
    "The bot never cold-calls, so at the 20 spots where hero faces an open with nothing in"
    " BEYOND THE BLINDS the solve's call is published as a raise rather than as a call. 165"
    " cells move. Merged and not deleted: at 15 of those 20 spots, across 40 of the 165"
    " cells, a hand's whole weight sits on calling, and deleting would leave a hand with no"
    " answer at all. Both figures are over the 20 merging spots and nothing wider. 10 of"
    " the 20 are the SMALL BLIND, which has posted 0.5: a blind is posted rather than"
    " chosen, so calling from there is still money going in behind an opener and it merges"
    " like the rest. The big blind's defence is untouched, and not because it posted more -"
    " because it closes the action, so its call ends the betting into a heads-up pot at a"
    " price it is already half paying. The small blind's call does neither, the big blind"
    " being still behind it. Every call to a three-bet is untouched too, hero's own raise"
    " being already in."
    "\n\n"
