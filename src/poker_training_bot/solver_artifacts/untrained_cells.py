...
def quantised_uniform_row(width: int) -> tuple[float, ...]:
    ...
    Measured on the committed 13.5bb chart: all 187 untrained cells publish exactly this row and
    no trained cell does, the nearest trained row sitting 106 basis points away from it.
    """
    if width < 1:
...
def is_untrained_cell(action_weights: ActionWeights, arrival_ppb: int | None) -> bool:
    ...
    **The condition is "this row was never trained"; uniformity is only its symptom, and neither
    ...
    - *arriving reach* does not separate at all. Untrained cells arrive at 49 to 517 basis points
      while 901 trained cells arrive at a single basis point, so any reach floor cuts trained
      cells first and keeps every untrained one. Rejected on measurement.
    - *spot arrival alone* is necessary and nowhere near sufficient. All 17 spots carrying an
      untrained cell arrive at exactly zero - but so do 175 of the 284, and the worst of those 17
      still has 2 trained cells among its 169. Refusing on arrival would refuse 158 spots of
      trained cells and those 2, which is what the 2026-08-27 ruling exists to prevent: arrival is
    ...
    - *the row alone* is exact on this chart and unsafe in general. Regret matching starts uniform
      ...
      numbers. That catches all 187 and nothing else here, the nearest trained row sitting 106
      basis points away. But at **width two the uniform row is `0.5, 0.5`**, which is ordinary
      poker rather than a signature, and a sibling fixture in `test_preflop_lookup.py` is exactly
      that. Shape alone would refuse a genuine coin-flip mix.
    ...
    187 cells across 17 spots that MAINT-34's decision 5 rules must refuse. A trained row cannot
    be caught, because a spot the solve reaches has `arrival_ppb > 0`.

    The shape half is an exact equality against a value the exporter computes, not a threshold -
    a tolerance would also swallow the 5 cells that sit within 200 basis points of uniform while
