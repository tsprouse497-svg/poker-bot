...
- **Two readings of `street_bet` are now both committed.** `STREET-BET-MEANING-AMBIGUOUS`. This phase's generator uses the street's bet level, which is the reading the preflop chart needs; the Phase 03 generator still passes hero's own contribution. *Pointer added 2026-08-21 by Phase 13: both readings are now separate fields. The street's level is `StrategyQuery.current_bet` and a seat's own share of it is `street_bet` on the per-seat `seat_states` record, so the field this bullet describes no longer exists under that name. This packet keeps the name it shipped with, being dated evidence rather than a document tracking the current tree; see `reports/phase_audits/PHASE_13_TABLE_STATE.md`.*
