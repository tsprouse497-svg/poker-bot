...
INVENTORY_OUTPUT = REPO_ROOT / "reports" / "active" / "latest_refusal_inventory.txt"
...
def inventory_lines(results: dict[str, SimulationResult]) -> list[str]:
    """Every spot the strategies could not answer, most-reached first.

    Keyed by the refusal's own detail rather than by anything re-derived here, so each entry
    names the chart cell the lookup actually asked about. Its own file because the diff
    between runs is the point: a gap that closes disappears, a gap that opens appears, and no
    aggregate percentage carries that signal.
    """
    lines = [
        "Refusal Inventory",
        "=================",
        "",
        f"Seed: {SEED}    Hands per run: {HANDS}    Table: {REQUIRED_SEATS}-handed"
        f" at {REQUIRED_DEPTH_BB}bb",
        "",
        *wrapped(
            "Every spot below is one the committed charts could not answer. This is the work"
            " list: each row names a spot key and a hand class, which together identify a"
            " chart cell somebody can fill, and the count is how many hands in this run"
            " reached it. Most-reached first, because a spot reached seventeen times is worth"
            " committing before one reached once."
        ),
        "",
        *wrapped(
            "What this is not: a census of the charts. It reports only the spots this run's"
            " seed and seating actually reached, so it is a lower bound on the gap. A spot"
            " missing from this file has either been covered or has not come up."
        ),
        "",
        *wrapped(
            "The spot keys are the lookup's own, taken from the refusal rather than derived"
            " again here, so a row names the cell the chart was really asked about."
        ),
    ]
    for label, result in results.items():
        ...
        lines += [
            ...
            f"## {label}",
            "",
            f"  hands dealt      {result.hands_dealt():>6}",
            f"  hands refused    {refused:>6}",
            f"  distinct spots   {len(inventory):>6}",
            "",
        ]
        if not inventory:
            lines += wrapped(
                "Nothing refused in this seating. That is not the charts being complete: an"
                " opponent that folds to everything keeps the betting tree shallow, so the"
                " uncovered spots are never reached.",
                indent="  ",
            )
            continue
        lines.append(
            f"  {'hands':>6}  {'spot key':<40}{'hands seen':>11}  reason"
        )
        for spot in inventory:
            seen = len(spot.hand_classes)
            lines.append(
                f"  {spot.hands:>6}  {spot.spot_key:<40}{seen:>11}  "
                f"{spot.code.rsplit(':', 1)[-1]}"
            )
        lines += ["", "  hand classes reached, per spot:"]
        for spot in inventory:
            shown = ", ".join(
                f"{name}x{count}" if count > 1 else name for name, count in spot.hand_classes
            )
            lines += wrapped(f"{spot.spot_key}: {shown or '-'}", indent="    ")
        lines += [
            "",
            *wrapped(
                f"{sum(spot.hands for spot in inventory)} refused hands across"
                f" {len(inventory)} distinct spots. Those two numbers must agree with the"
                " hands-refused count above; if they do not, refusal detail is being dropped"
                " somewhere between the strategy and this file.",
                indent="  ",
            ),
        ]
    return lines
...
def render_inventory(results: dict[str, SimulationResult]) -> str:
    body = inventory_lines(results)
    body += ["", "Generated by `scripts/generate_profile_comparison_report.py`."]
    return "\n".join(body) + "\n"


def render(self_play: SimulationResult, floor: SimulationResult) -> str:
    sections = [
        header_lines(),
...
def main() -> int:
    ...
    REPORT_OUTPUT.write_text(render(self_play, floor), encoding="utf-8")
    INVENTORY_OUTPUT.write_text(
        render_inventory({"six copies of the chart bot": self_play,
                          "chart bot against five check-fold seats": floor}),
        encoding="utf-8",
    )
    print(f"wrote {REPORT_OUTPUT.relative_to(REPO_ROOT)}")
    print(f"wrote {INVENTORY_OUTPUT.relative_to(REPO_ROOT)}")
    return 0
